...
Runnable examples for each public POET primitive. Each file is a self-contained
translation unit built and linked from the docs.
...
The Google Benchmark example is opt-in so the default build stays
dependency-free. Enable it with `-DPOET_BUILD_EXAMPLE_BENCHMARK=ON`.
...
| [`polynomial.cpp`](polynomial.cpp) | Horner specialized on degree (`dispatch` + `static_for`) | | [![CE][ce-badge]](https://flatironinstitute.github.io/poet/polynomial.html) |
